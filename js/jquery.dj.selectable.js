...
function bindSelectables(context) {
    jQuery(":input[data-selectable-type=text]", context).djselectable();
    jQuery(":input[data-selectable-type=combobox]", context).djselectable();
    jQuery(":input[data-selectable-type=hidden]", context).each(function(i, elem) {
        var hiddenName = $(elem).attr('name');
        var textName = hiddenName.replace('_1', '_0');
        jQuery(":input[name=" + textName + "][data-selectable-url]").bind(
